...
PIG_to_WHPE_share = function(file_path, path_out,userID = "IMASUTASKB",row_start = 1,row_end = NA, t_thresh = 6, d_thresh = 1000){
  ...
  ctd_path = "https://data.imas.utas.edu.au/attachments/3e0afabb-ebd2-4972-bb67-8626c2643129/reformatted_data/profiling_sensors/"

  df <- readHTMLTable(httr::content(httr::GET("https://data.imas.utas.edu.au/attachments/3e0afabb-ebd2-4972-bb67-8626c2643129/reformatted_data/profiling_sensors/"), "text"))

  ctd_files = basename(grep(".csv", df$`NULL`$Name, value = TRUE, fixed = TRUE))


  ctd_path2 = "https://github.com/KimBaldry/BIO-MATE/tree/main/product_data/next_version/reformatted_data"
  repos = "KimBaldry/BIO-MATE"
  req <- httr::GET(paste("https://api.github.com/repos", repos,
  ...
  ctd_files2 = basename(grep("product_data/next_version/reformatted_data/profiling_sensors/", filelist, value = TRUE, fixed = TRUE))

  if(length(ctd_files2) > 0){ctd_files = c(ctd_files,ctd_files2)}
  ...
  for(rw in row_end)
  {
    ...
    vars = colnames(info)[-c(1:which(colnames(info) == "Method"))]
    ...
    if(is.na(info$Underway_ID)){info$Underway_ID = ""}
    ...
    for(fl in files){
            ...
            if(info$file_type == "text delim"){
              ...
              if(is.na(info$delim)){info$delim = ""}
              if(info$delim == "rect"){
